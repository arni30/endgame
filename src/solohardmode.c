...
int solohardmode(void)
{
    ...
    while (SDL_PollEvent(&event))
        ...
        if (y_pos + platform_dest.w >= y_platform && y_pos <= y_platform + platform_dest.h)
        {
            ...
            if (x_pos + platform_dest.w < x_platform + platform_dest.w)
            {
                x_vel = -x_vel;
                //y_vel = y_vel * 10;        
                if (y_speed > 0 && y_vel > 0) y_vel *= 1.2;
                if (y_speed < 0 && y_vel < 0) y_vel *= 1.2;
                if (y_speed > 0 && y_vel < 0) y_vel /= 1.4;
                if (y_speed < 0 && y_vel > 0) y_vel /= 1.4;

                // if (x_pos + platform_dest.w >= x_platform && x_pos <= x_platform + platform_dest.w)
                // {
                //     if (y_pos + platform_dest.w >= y_platform / 3 && y_pos <= y_platform / 3 + platform_dest.h)
                //     {
                //         y_vel = y_vel * 10;
                //     }
                // }
        ...
        if (y_pos + bot_dest.w >= y_bot && y_pos <= y_bot + bot_dest.h)
        {
            ...
            if (x_pos + bot_dest.w < x_bot + bot_dest.w)
            {
                ...
                // vertical collision
                x_vel = -x_vel;
                if (y_speed_bot > 0 && y_vel > 0) y_vel *= 1.2;
                if (y_speed_bot < 0 && y_vel < 0) y_vel *= 1.2;
                if (y_speed_bot > 0 && y_vel < 0) y_vel /= 1.4;
                if (y_speed_bot < 0 && y_vel > 0) y_vel /= 1.4;
            }
            // else
            // {
    ...
    //     if(y_bot - y_pos > 4 || y_bot - y_pos < -4){
    //     if(y_bot < y_pos){
    //         y_speed_bot = SPEED;
    //     }
    //      if(y_bot > y_pos){
    ...
    // }
        ...
        if (botup && !botdown) y_speed_bot = SPEED;
        if (botdown && !botup) y_speed_bot = -SPEED;
